# Assembly documentation

The following pages document the JC16 assembly language and the JCASM (JC16 assembler) reference implementation and its usage.

## Table of contents

- [instruction syntax](./syntax.md)
- [preprocessor directives](./preprocessor/index.md)
    - [definitions](./preprocessor/definitions.md)
    - [scopes](./preprocessor/scopes.md)
- [JCASM](./jcasm/index.md)
    - [assembler flags](./jcasm/flags.md)
    - [optimization](./jcasm/optimization.md)
    - [output formats](./jcasm/formats.md)
- [comments](./comments.md)
- [best-practices](./best-practices.md)
